...
async function calculateTrends(clientId: string, dbHelper: ReturnType<typeof dbApiHelper>) {
  try {
    ...
    // Oblicz wzrosty
    const currentSquareMeters = completedRecentQuotes.reduce((sum, quote: any) => sum + quote.area, 0)
    const previousSquareMeters = completedPreviousQuotes.reduce((sum, quote: any) => sum + quote.area, 0)

    const currentSavings = completedRecentQuotes.reduce((sum, quote: any) => {
      const avgPricePerM2 = (quote.price_min + quote.price_max) / 2
      return sum + (avgPricePerM2 * quote.area)
    }, 0)

    const previousSavings = completedPreviousQuotes.reduce((sum, quote: any) => {
      const avgPricePerM2 = (quote.price_min + quote.price_max) / 2
      return sum + (avgPricePerM2 * quote.area)
    }, 0)
